...
/**
 *一个连接提供了一个方便的接口来从网页内容，并解析成文档。
  *<P>
  *要获得一个新的连接，使用{@link org.jsoup.Jsoup＃连接（字符串）}。连接包含{@link请求}
  *和{@link响应}对象。请求对象是可重复使用的原型请求。
  *<P>
  *重配置可以利用制成无论是在连接的快捷方式（例如{@link#userAgent（字符串）}），
  *或通过直接在Connection.Request对象的方法。所有的请求配置必须请求之前进行
  *被执行。
 */
